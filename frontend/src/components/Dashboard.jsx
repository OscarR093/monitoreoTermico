...
const TemperatureCard = ({ equipment, onClick }) => {
  ...
  const getTemperatureColor = (temp) => {
    if (!isValidTemp) return 'text-gray-400'
    if (temp <= 300) return 'text-blue-600'
    if (temp <= 600) return 'text-green-600'
    if (temp <= 800) return 'text-yellow-600'
    return 'text-red-600'
  }

  const getBackgroundColor = (temp) => {
    if (!isValidTemp) return 'bg-gray-50 border-gray-200'
    if (temp <= 300) return 'bg-blue-50 border-blue-200 hover:bg-blue-100'
    if (temp <= 600) return 'bg-green-50 border-green-200 hover:bg-green-100'
    if (temp <= 800) return 'bg-yellow-50 border-yellow-200 hover:bg-yellow-100'
  ...
  return (
    <div
      onClick={onClick}
      className={`
        relative p-6 rounded-xl border-2 cursor-pointer transition-all duration-300 transform hover:scale-105 hover:shadow-lg
        ${getBackgroundColor(equipment.temperature)}
      `}
    >
      <div className='flex flex-col items-center justify-center h-full'>
        ...
        <div className={`text-4xl font-black mb-2 ${getTemperatureColor(equipment.temperature)}`}>
          {isValidTemp ? `${equipment.temperature}°C` : '---'}
        </div>
        
        <div className={`text-sm font-medium ${isValidTemp ? 'text-gray-600' : 'text-gray-400'}`}>
          {isValidTemp ? 'Conectado' : 'Desconectado'}
        </div>
        
        {/* Indicador de estado */}
        <div className={`
          absolute top-3 right-3 w-3 h-3 rounded-full
          ${isValidTemp ? 'bg-green-500' : 'bg-gray-400'}
        `} />
      </div>
    </div>
  )
    ...
    <div className='flex flex-col h-screen font-sans bg-gradient-to-br from-gray-50 to-gray-100 text-gray-800'>
      ...
      <header className='bg-white p-4 shadow-md flex justify-between items-center fixed top-0 left-0 w-full z-30'>
        ...
        <div className='hidden sm:flex flex-grow justify-center'>
          <h1 className='text-xl font-bold'>Dashboard de Temperaturas</h1>
        </div>
      ...
      {/* --- Panel de Menú Móvil --- */}
      <div className={`fixed inset-0 bg-black bg-opacity-50 z-40 transition-opacity md:hidden ${mainMenuOpen ? 'opacity-100' : 'opacity-0 pointer-events-none'}`} onClick={toggleMainMenu} />
      ...
      <main className='flex-1 w-full mt-20 p-4 md:p-6 overflow-y-auto'>
        <div className='max-w-7xl mx-auto'>
          <div className='mb-6'>
            <h2 className='text-2xl font-bold text-gray-800 mb-2'>Monitoreo en Tiempo Real</h2>
            <p className='text-gray-600'>Haz clic en cualquier equipo para ver detalles</p>
          </div>
